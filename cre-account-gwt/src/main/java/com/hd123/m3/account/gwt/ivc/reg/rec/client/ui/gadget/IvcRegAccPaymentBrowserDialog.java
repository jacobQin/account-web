 ...
 * 
 */
public class IvcRegAccPaymentBrowserDialog extends RBrowserDialog<BAccountPayment> {
  ...
  public IvcRegAccPaymentBrowserDialog() {
    super();
    drawCol();
  ...
  private class PageDataProvider implements RPageDataProvider<BAccountPayment>, RFilterCallback {
    ...
    public void onQuery() {
      ...
      filter.setPaymentBillNumber(billNumberField.getValue());
    ...
    public void fetchData(int page, int pageSize, String sortField, OrderDir sortDir,
      ...
      if (accountUnitControl && StringUtil.isNullOrBlank(filter.getAccountUnitUuid())) {
        callback.onSuccess(new RPageData<BAccountPayment>());
        return;
      ...
      if (searchForm.isValid() == false) {
        RPageData<BAccountPayment> oldData = new RPageData<BAccountPayment>();
      ...
      InvoiceRegService.Locator.getService().queryAccPayments(filter, callback);
    }

    @Override
    public Object getData(int row, int col, BAccountPayment rowData, List<BAccountPayment> pageData) {
