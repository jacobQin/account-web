...
import com.hd123.m3.account.gwt.payment.pay.client.ui.gadget.PaymentConfigDialog;
import com.hd123.m3.account.gwt.payment.pay.intf.client.PaymentUrlParams;
import com.hd123.m3.account.gwt.payment.pay.intf.client.PaymentUrlParams.Search;
 ...
 * 
 */
public class PaymentSearchPage extends BaseBpmSearchPage implements Search, BeforePrintHandler {

  public static EmbeddedFieldDef counterpart = new EmbeddedFieldDef(FIELD_COUNTERPART, EPPayment
      .getInstance().getFieldCaption(GRes.FIELDNAME_COUNTERPART, GRes.R.counterpart()));
  public static EmbeddedFieldDef accountUnit = new EmbeddedFieldDef(FIELD_ACCOUNTUNIT, EPPayment
      .getInstance().getFieldCaption(GRes.FIELDNAME_BUSINESS, GRes.R.business()));
  public static DateFieldDef paymentDate = new DateFieldDef("paymentDate",
      PaymentMessages.M.paymentDate(), false, null, true, null, true, GWTFormat.fmt_yMd, false);
  public static EmbeddedFieldDef permGroup = new EmbeddedFieldDef(FIELD_PERMGROUP,
      PaymentMessages.M.permGroup());

  public static EmbeddedFieldDef counterpartType = new EmbeddedFieldDef(
      PaymentUrlParams.Flecs.FIELD_COUNTERPARTTYPE, EPPayment.getInstance().getFieldCaption(
          GRes.FIELDNAME_COUNTERPART_TYPE, GRes.R.counterpartType()));

  private EPPayment ep = EPPayment.getInstance();
  private static PaymentSearchPage instance;

  public static PaymentSearchPage getInstance() {
    if (instance == null) {
      instance = new PaymentSearchPage();
    }
    return instance;
  }
  ...
  private PaymentConfigDialog configDialog;
  ...
  private void drawToolbar() {
    ...
    moreButton = new RToolbarMenuButton(PaymentMessages.M.operate(), moreMenu);
  ...
  private void drawLineMenu() {
    ...
    effectOneMenuItem = new RMenuItem(PaymentMessages.M.effect(), clickHandler);
    effectOneMenuItem.setHotKey(null);

    abortOneMenuItem = new RMenuItem(PaymentMessages.M.abort() + "...", clickHandler);
  ...
  private Widget drawGrid() {
    ...
    stateCol = new RGridColumnDef(PPaymentDef.finishBizState);
    ...
    defrayalTotalCol = new RGridColumnDef(PaymentMessages.M.defrayalTotal());
  ...
  private void drawFlecsPanel() {
    ...
    flecsPanel.addField(PPaymentDef.finishBizState);
    ...
    flecsPanel.addField(paymentDate);
  ...
  private class CounterpartTypeField extends RComboBox<String> {
    ...
    public CounterpartTypeField(FieldDef fieldDef) {
      ...
      setNullOptionText(PaymentMessages.M.all());
      setEditable(false);
  ...
  private class GridDataProvider implements RPageDataProvider<BPayment> {
    ...
    public void fetchData(int page, int pageSize, String sortField, OrderDir sortDir,
      ...
      PaymentService.Locator.getService().query(flecsPanel.getCurrentConfig().getConditions(),
          pageSort, callback);
    }
  ...
  private void refreshCommonds() {
    configAction.setVisible(ep.isPermitted(PaymentPermDef.CONFIG));
    deleteAction.setVisible(ep.isPermitted(PaymentPermDef.DELETE));
    effectAction.setVisible(ep.isPermitted(PaymentPermDef.EFFECT));
    abortAction.setVisible(ep.isPermitted(PaymentPermDef.ABORT));
  ...
  private void refreshTitle() {
    ...
    ep.getTitleBar().setTitleText(PaymentMessages.M.search());
  ...
  private List<Condition> getDefaultConditions() {
    ...
    conditions.add(new Condition(PPaymentDef.finishBizState, DefaultOperator.EQUALS, null));
    ...
    conditions.add(new Condition(paymentDate, DefaultOperator.EQUALS, null));
  ...
  private void doDelete() {
    if (pagingGrid.getSelections().isEmpty()) {
      RMsgBox.showError(PaymentMessages.M.seleteDataToAction(PaymentMessages.M.delete(),
          PaymentMessages.M.payment()));
      return;
    }

    batchProcessor.batchProcess(PaymentMessages.M.delete(), new BPayment[] {},
        new PagingGridBatchProcessCallback<BPayment>() {
          ...
          public void execute(BPayment bill, BatchProcesser<BPayment> processer,
            ...
            PaymentService.Locator.getService().delete(bill.getUuid(), bill.getVersion(), callback);
            BPaymentLogger.getInstance().log(PaymentMessages.M.delete(), bill);
          }
        });
  }
  ...
  private void doEffect() {
    if (pagingGrid.getSelections().isEmpty()) {
      RMsgBox.showError(PaymentMessages.M.seleteDataToAction(PaymentMessages.M.effect(),
          PaymentMessages.M.payment()));
      return;
    }

    batchProcessor.batchProcess(PaymentMessages.M.effect(), new BPayment[] {},
        new PagingGridBatchProcessCallback<BPayment>() {
          @Override
          public void execute(BPayment bill, BatchProcesser<BPayment> processer,
              AsyncCallback callback) {
            ...
            PaymentService.Locator.getService().effect(bill.getUuid(), null, bill.getVersion(),
                callback);
            BPaymentLogger.getInstance().log(PaymentMessages.M.effect(), bill);
          }
        });
  }
  ...
  private void doAbortConfirm() {
    if (pagingGrid.getSelections().isEmpty()) {
      RMsgBox.showError(PaymentMessages.M.seleteDataToAction(PaymentMessages.M.abort(),
          PaymentMessages.M.payment()));
      return;
    }

    InputBox.show(PaymentMessages.M.abortReason(), null, true, PPaymentDef.latestComment,
  ...
  private void doAbort(final String reason) {
    batchProcessor.batchProcess(PaymentMessages.M.abort(), new BPayment[] {}, true, false,
        new PagingGridBatchProcessCallback<BPayment>() {
          @Override
          public void execute(BPayment bill, BatchProcesser<BPayment> processer,
              AsyncCallback callback) {
            ...
            PaymentService.Locator.getService().abort(bill.getUuid(), reason, bill.getVersion(),
                callback);
            BPaymentLogger.getInstance().log(PaymentMessages.M.abort(), bill);
          }
        });
  }
  ...
  private void doConfig() {
    if (configDialog == null) {
      configDialog = new PaymentConfigDialog();
    }
    configDialog.center();
  }
  ...
  private void doEditOne(ClickEvent event) {
    ...
    JumpParameters jumParams = new JumpParameters(PaymentEditPage.START_NODE);
    jumParams.getUrlRef().set(PaymentEditPage.PN_UUID, bill.getUuid());
    ep.jump(jumParams);
  }
  ...
  private void deleteOne(final BPayment bill) {
    RLoadingDialog
        .show(PaymentMessages.M.beDoing(PaymentMessages.M.delete(), bill.getBillNumber()));
    PaymentService.Locator.getService().delete(bill.getUuid(), bill.getVersion(),
        new RBAsyncCallback2() {
          ...
          public void onSuccess(Object result) {
            ...
            String msg = PaymentMessages.M.onSuccess(PaymentMessages.M.delete(),
                PaymentMessages.M.payment(), bill.getBillNumber());
  ...
  private void effectOne(final BPayment bill) {
    RLoadingDialog
        .show(PaymentMessages.M.beDoing(PaymentMessages.M.effect(), bill.getBillNumber()));
    PaymentService.Locator.getService().effect(bill.getUuid(), null, bill.getVersion(),
        new RBAsyncCallback2() {
  ...
  private void abortOne(final BPayment bill, String comment) {
    RLoadingDialog.show(PaymentMessages.M.beDoing(PaymentMessages.M.abort(), bill.getBillNumber()));
    PaymentService.Locator.getService().abort(bill.getUuid(), comment, bill.getVersion(),
        new RBAsyncCallback2() {
  ...
  protected void appendLineMenu(RPopupMenu menu, Object bill) {
    ...
    boolean canEdit = ep.isPermitted(PaymentPermDef.UPDATE);
    boolean canDelete = ep.isPermitted(PaymentPermDef.DELETE);
    boolean canEffect = ep.isPermitted(PaymentPermDef.EFFECT);
    boolean canAbort = ep.isPermitted(PaymentPermDef.ABORT);
