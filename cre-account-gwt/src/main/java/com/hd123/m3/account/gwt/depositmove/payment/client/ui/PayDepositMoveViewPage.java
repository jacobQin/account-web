 ...
 * 文件名：	PayDepositMoveViewPage.java
 * 模块说明：	
 * 修改历史：
...
import com.hd123.m3.account.gwt.depositmove.payment.client.rpc.PayDepositMoveLoader;
...
import com.hd123.m3.account.gwt.depositmove.payment.intf.client.PayDepositMoveUrlParams.View;
 ...
 * 
 */
public class PayDepositMoveViewPage extends BaseBpmViewPage implements View {
  private static PayDepositMoveViewPage instance = null;

  public static PayDepositMoveViewPage getInstance() {
    if (instance == null)
      instance = new PayDepositMoveViewPage();
    return instance;
  }

  public PayDepositMoveViewPage() {
    super();
    drawToolbar();
    drawSelf();
  }
  ...
  private DispatchLinkField outContractNumberField;
  ...
  private DispatchLinkField inContractNumberField;
  ...
  private Widget drawOutInfoPanel() {
    ...
    outAccountUnitField = new RViewStringField(getEP().getFieldCaption(GRes.FIELDNAME_BUSINESS,
        GRes.R.business()));
    outInfoForm.addField(outAccountUnitField);

    outCounterpartField = new RViewStringField(getEP().getFieldCaption(GRes.FIELDNAME_COUNTERPART,
        GRes.R.counterpart()));
    outInfoForm.addField(outCounterpartField);

    outContractNumberField = new DispatchLinkField(PDepositMoveDef.constants.outContract_code());
    outContractNumberField.setLinkKey(GRes.R.dispatch_key());
    outInfoForm.addField(outContractNumberField);
  ...
  private Widget drawInInfoPanel() {
    ...
    inAccountUnitField = new RViewStringField(getEP().getFieldCaption(GRes.FIELDNAME_BUSINESS,
        GRes.R.business()));
    inInfoForm.addField(inAccountUnitField);

    inCounterpartField = new RViewStringField(getEP().getFieldCaption(GRes.FIELDNAME_COUNTERPART,
        GRes.R.counterpart()));
    inInfoForm.addField(inCounterpartField);

    inContractNumberField = new DispatchLinkField(PDepositMoveDef.constants.inContract_code());
    inContractNumberField.setLinkKey(GRes.R.dispatch_key());
    inInfoForm.addField(inContractNumberField);

    inContractNameField = new RViewStringField(PDepositMoveDef.constants.inContract_name());
  ...
  public void onShow(final JumpParameters params) {
    ...
    PayDepositMoveLoader.decodeParams(params, new Command() {

      @Override
      public void execute() {
        entity = PayDepositMoveLoader.getEntity();
        refresh();
      }
    });
  }
  ...
  private void refreshEntity() {
    ...
    if (entity.getOutContract() == null) {
      outContractNumberField.setValue(entity.getOutContract() == null ? null : entity
      ...
      if (entity.getOutCounterpart() != null
          && BCounterpart.COUNPERPART_PROPRIETOR.equals(entity.getOutCounterpart()
              .getCounterpartType())) {
        outContractNumberField.setValue(GRes.FIELDNAME_LEASEBACKCONTRACT_BILLTYPE, entity
            .getOutContract().getCode());
      } else {
        outContractNumberField.setValue(GRes.FIELDNAME_CONTRACT_BILLTYPE, entity.getOutContract()
            .getCode());
      }
    }
    ...
    if (entity.getInContract() == null) {
      inContractNumberField.setValue(entity.getInContract() == null ? null : entity.getInContract()
          .getCode());
    } else {
      if (entity.getInCounterpart() != null
          && BCounterpart.COUNPERPART_PROPRIETOR.equals(entity.getInCounterpart()
              .getCounterpartType())) {
        inContractNumberField.setValue(GRes.FIELDNAME_LEASEBACKCONTRACT_BILLTYPE, entity
            .getInContract().getCode());
      } else {
        inContractNumberField.setValue(GRes.FIELDNAME_CONTRACT_BILLTYPE, entity.getInContract()
            .getCode());
      }
    }
  ...
  private class Handler_editAction implements ClickHandler {
    @Override
    public void onClick(ClickEvent event) {
      JumpParameters params = new JumpParameters(PayDepositMoveUrlParams.Edit.START_NODE);
      params.getUrlRef().set(PayDepositMoveUrlParams.Edit.PN_ENTITY_UUID, entity.getUuid());
      getEP().jump(params);
    }
  }
  ...
  private void doDelete() {
    ...
    PayDepositMoveService.Locator.getService().remove(entity.getUuid(), entity.getVersion(),
  ...
  private void doEffect() {
    ...
    PayDepositMoveService.Locator.getService().effect(entity.getUuid(), null, entity.getVersion(),
        new RBAsyncCallback2() {

          @Override
          public void onException(Throwable caught) {
            RLoadingDialog.hide();
            String msg = DepositMoveMessage.M.actionFailed(DepositMoveMessage.M.effect(), getEP()
                .getModuleCaption());
            RMsgBox.showError(msg, caught);
          }
  ...
  private void doAbort() {
    ...
    PayDepositMoveService.Locator.getService().abort(entity.getUuid(), null, entity.getVersion(),
        new RBAsyncCallback2() {

          @Override
          public void onException(Throwable caught) {
            RLoadingDialog.hide();
            String msg = DepositMoveMessage.M.actionFailed(DepositMoveMessage.M.abort(), getEP()
                .getModuleCaption());
            RMsgBox.showError(msg, caught);
          }
  ...
  private class Handler_viewLogAction implements ClickHandler {
    @Override
    public void onClick(ClickEvent event) {
      JumpParameters params = new JumpParameters(PayDepositMoveLogViewPage.START_NODE);
      params.getUrlRef().set(PayDepositMoveLogViewPage.PN_ENTITY_UUID, entity.getUuid());
      getEP().jump(params);
    }
  }
  ...
  private class ProcessDialog extends M3BaseOutgoingDialog {
    ...
    protected void doExcute(BOperation operation, AsyncCallback<Object> callback) {
      ...
      PayDepositMoveServiceAgent.executeTask(processCtx, operation, entity, false, this);
    }
  }
  ...
  protected void refreshToolButtons() {
    ...
    totalField.setVisible(isEffect);
