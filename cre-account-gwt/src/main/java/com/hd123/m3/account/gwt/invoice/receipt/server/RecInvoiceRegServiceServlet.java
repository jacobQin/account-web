 ...
 */
package com.hd123.m3.account.gwt.invoice.receipt.server;
...
import com.hd123.m3.account.gwt.invoice.receipt.server.querybuilder.RecInvoiceRegQueryBuilder;
 ...
 * 
 */
public class RecInvoiceRegServiceServlet extends AccBPMServiceServlet implements
    RecInvoiceRegService {
  ...
  protected Set<BPermission> getAuthorizedPermissions(Operator sessionUser) {
    ...
    Set<BPermission> permissions = helper.getAuthorizedPermissions(sessionUser.getQualifiedId(),
        RecInvoiceRegPermDef.RESOURCE_RECINVOICEREG_SET);
    return permissions;
  }
  ...
  public BInvoiceRegConfig getConfig() throws ClientBizException {
    try {
      ...
      String regTotalWriteable = getOptionService().getOption(InvoiceRegs.OPTION_PATH,
          InvoiceRegs.KEY_REC_REGTOTAL_WRITABLE);
      ...
      String taxDiffHi = getOptionService().getOption(InvoiceRegs.OPTION_PATH,
          InvoiceRegs.KEY_REC_TAX_DIFF_HI);
      ...
      String taxDiffLo = getOptionService().getOption(InvoiceRegs.OPTION_PATH,
          InvoiceRegs.KEY_REC_TAX_DIFF_LO);
      ...
      String totalDiffHi = getOptionService().getOption(InvoiceRegs.OPTION_PATH,
          InvoiceRegs.KEY_REC_TOTAL_DIFF_HI);
      ...
      String totalDiffLo = getOptionService().getOption(InvoiceRegs.OPTION_PATH,
          InvoiceRegs.KEY_REC_TOTAL_DIFF_LO);
  ...
  private FlecsQueryDefinition buildQueryDefinition(List<Condition> conditions,
    ...
    queryDef.getFlecsConditions().addAll(
        RecInvoiceRegQueryBuilder.getInstance().buildFlecsConditions(conditions));
    queryDef.setOrders(RecInvoiceRegQueryBuilder.getInstance().buildQueryOrders(sortOrders));
  ...
  private FlecsQueryDefinition buildDefaultQueryDefinition() throws ClientBizException {
    ...
    queryDef.addCondition(InvoiceRegs.CONDITION_DIRECTION_EQUALS, Direction.RECEIPT);
  ...
  public BInvoiceReg create() throws ClientBizException {
    try {
      InvoiceReg source = getInvoiceRegService().create(Direction.RECEIPT);
  ...
  public BInvoiceReg save(BInvoiceReg entity, BProcessContext task) throws ClientBizException {
    try {
      ...
      if (entity.getUuid() == null)
        target = getInvoiceRegService().create(Direction.RECEIPT);
      else
        target = getInvoiceRegService().get(entity.getUuid(), InvoiceReg.PART_WHOLE);

      write(entity, target);

      String invoiceReg = getInvoiceRegService().save(target,
          ConvertHelper.getOperateContext(getSessionUser()));

      target = getInvoiceRegService().get(invoiceReg);
  ...
  public interface R {
    ...
    @DefaultStringValue("指定的\"收款发票登记单\"不存在。")
    String nullObject();
  }
