...
import com.hd123.m3.account.gwt.invoice.receipt.client.ui.gadget.RecInvoiceRegGeneralEditGadget;
...
import com.hd123.m3.account.gwt.invoice.receipt.intf.client.RecInvoiceRegUrlParams.Edit;
 ...
 * 
 */
public class RecInvoiceRegEditPage extends BaseBpmEditPage implements Edit, RValidatable,
    RActionHandler, HasRActionHandlers, SelectionHandler<Message> {
  private static RecInvoiceRegEditPage instance = null;

  public static RecInvoiceRegEditPage getInstance() {
    if (instance == null) {
      instance = new RecInvoiceRegEditPage();
    }
    return instance;
  }

  public RecInvoiceRegEditPage() {
    super();
    drawToolbar();
    drawSelf();
    afterDraw();
  ...
  private RecInvoiceRegGeneralEditGadget generalGadget;
  ...
  private void drawSelf() {
    ...
    generalGadget = new RecInvoiceRegGeneralEditGadget();
  ...
  private boolean checkIn() {
    if (getEP().isProcessMode())
      return true;
    if (getEP().isPermitted(RecInvoiceRegPermDef.UPDATE) == false) {
  ...
  public void onSelection(SelectionEvent<Message> event) {
    if (event.getSelectedItem().getSource() instanceof InvoiceRegLocator) {
      ...
      if (locator.getLocatorType().equals(InvoiceRegLocator.lineType)) {
        RActionEvent.fire(RecInvoiceRegEditPage.this, InvoiceRegActionName.ONSELECT_LINE,
            Integer.valueOf(locator.getLineNumber() - 1));
      } else if (locator.getLocatorType().equals(InvoiceRegLocator.invoiceType)) {
        RActionEvent.fire(RecInvoiceRegEditPage.this, InvoiceRegActionName.ONSELECT_INVOICE,
            Integer.valueOf(locator.getLineNumber() - 1));
      }
    }
  }
    ...
    RecInvoiceRegService.Locator.getService().save(entity, getEP().getProcessCtx(),
        new RBAsyncCallback2<BInvoiceReg>() {
          ...
          public void onSuccess(BInvoiceReg result) {
            RLoadingDialog.hide();
            BInvoiceRegLogger.getInstance().log(InvoiceRegMessage.M.modify(), entity);
    ...
    public void onClick(ClickEvent event) {
      if (event.getSource() == addInvoiceAction) {
        ...
        Scheduler.get().scheduleDeferred(new ScheduledCommand() {
          @Override
          public void execute() {
            RActionEvent.fire(RecInvoiceRegEditPage.this, InvoiceRegActionName.ADD_INVOICE);
          }
        });
      } else if (event.getSource() == insertInvoiceAction) {
        RActionEvent.fire(RecInvoiceRegEditPage.this, InvoiceRegActionName.INSERT_INVOICE);
      } else if (event.getSource() == deleteInvoiceAction) {
        RActionEvent.fire(RecInvoiceRegEditPage.this, InvoiceRegActionName.BATCH_DELETE_INVOICE);
      } else if (event.getSource() == deleteLineAction) {
        RActionEvent.fire(RecInvoiceRegEditPage.this, InvoiceRegActionName.BATCH_DELETE_LINE);
  ...
  public void refreshToolbar(RToolbar toolbar) {
    saveAction.setVisible(getEP().isPermitted(RecInvoiceRegPermDef.UPDATE));
  }
