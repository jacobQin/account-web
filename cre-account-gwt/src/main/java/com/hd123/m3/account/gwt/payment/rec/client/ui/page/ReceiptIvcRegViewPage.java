...
import com.hd123.m3.account.gwt.payment.rec.intf.client.ReceiptUrlParams.IvcRegView;
 ...
 * 
 */
public class ReceiptIvcRegViewPage extends BaseContentPage implements IvcRegView {

  private OtherFieldDef dateRangeDef = new OtherFieldDef("dateRange", CommonMessages.M.dateRange(),
      true);
  private StringFieldDef ivcCodeDef = new StringFieldDef("ivcCode", ReceiptMessages.M.ivcCode(),
      true, 0, 32);
  private StringFieldDef ivcNumberDef = new StringFieldDef("ivcNumber",
      ReceiptMessages.M.ivcNumber(), true, 0, 32);
  private BigDecimalFieldDef ivcTotalDef = new BigDecimalFieldDef("ivcTotal",
      ReceiptMessages.M.ivcTotal(), true, null, true, null, true, 2);
  private DateFieldDef ivcDateDef = new DateFieldDef("ivcDate", ReceiptMessages.M.ivcDate());
  private StringFieldDef ivcTypeDef = new StringFieldDef("ivcType", ReceiptMessages.M.ivcType(),
      true, 0, 20);

  private static ReceiptIvcRegViewPage instance;

  public static ReceiptIvcRegViewPage getInstance() {
    if (instance == null)
      instance = new ReceiptIvcRegViewPage();
    return instance;
  }

  public ReceiptIvcRegViewPage() {
    super();
    drawToolbar();
    drawSelf();
  }
  ...
  private void drawToolbar() {
    cancelAction = new RAction(ReceiptMessages.M.returnButton(), clickHandler);
  ...
  private Widget drawGeneral() {
    ...
    box.setCaption(ReceiptMessages.M.generalInfo());
    box.setWidth("100%");
    box.setContent(vp);
    return box;
  }
  ...
  private Widget drawBasicInfo() {
    ...
    billNumberField = new RViewStringField(ReceiptMessages.M.receiptNumber());
    billNumberField.addStyleName(RTextStyles.STYLE_BOLD);
    form.addField(billNumberField);

    storeField = new RViewStringField(ReceiptMessages.M.store());
    ...
    receiptDateField = new RViewDateField(ReceiptMessages.M.receiptDate());
    form.addField(receiptDateField);
  ...
  private Widget drawRegiste() {
    ...
    resultTotalHtml = new HTML(ReceiptMessages.M.resultTotal(0));
    ...
    RCaptionBox box = new RCaptionBox();
    box.setCaption(ReceiptMessages.M.registeInfo());
    box.setWidth("100%");
  ...
  public void onShow(final JumpParameters params) {
    ...
    final String uuid = params.getUrlRef().get(ReceiptIvcRegViewPage.PN_UUID);
    decodeParams(uuid, new Command() {
      @Override
      public void execute() {
        if (bill == null || bill.getAccountLines().isEmpty()) {
          RMsgBox.show(ReceiptMessages.M.noRegisteAccounts(), new RMsgBox.Callback() {
            @Override
  ...
  private void decodeParams(String uuid, final Command cmd) {
    ReceiptService.Locator.getService().loadRegiste(uuid, new AsyncCallback<BPayment>() {
      @Override
      public void onFailure(Throwable caught) {
        String msg = ReceiptMessages.M.actionFailed(ReceiptMessages.M.find(),
            ReceiptMessages.M.receipt());
  ...
  private void refreshTitle() {
    ...
    ep.getTitleBar().setTitleText(ReceiptMessages.M.receipt());
    ep.getTitleBar().appendAttributeText(ReceiptMessages.M.ivcRegiste());
  }
  ...
  private class GridDataProvider implements RGridDataProvider {
    ...
    public Object getData(int row, int col) {
      if (bill == null || bill.getAccountLines() == null || bill.getAccountLines().isEmpty()
        ...
        return null;
      resultTotalHtml.setText(ReceiptMessages.M.resultTotal(bill.getAccountLines().size()));
