 ...
 * 文件名：	InvoiceTransportDetailGadget.java
 * 模块说明：	
 * 修改历史：
 ...
 * 
 */
public class InvoiceTransportDetailGadget extends RCaptionBox implements RActionHandler {
  ...
  public InvoiceTransportDetailGadget() {
    super();
  ...
  private boolean validateContain() {
    ...
    boolean result = true;
    for (int i = 0; i < entity.getTransportLines().size(); i++) {
      for (int j = i + 1; j < entity.getTransportLines().size(); j++) {
        if ((!StringUtil.isNullOrBlank(entity.getTransportLines().get(i).getStartNumber()))
            && (!StringUtil.isNullOrBlank(entity.getTransportLines().get(i).getEndNumber()))
            && (!StringUtil.isNullOrBlank(entity.getTransportLines().get(j).getStartNumber()))
            && (!StringUtil.isNullOrBlank(entity.getTransportLines().get(j).getEndNumber()))
            && isNumeric(entity.getTransportLines().get(i).getStartNumber())
            && isNumeric(entity.getTransportLines().get(i).getEndNumber())
            && isNumeric(entity.getTransportLines().get(j).getStartNumber())
            && isNumeric(entity.getTransportLines().get(j).getEndNumber())) {
          isValid = !isInclude(entity.getTransportLines().get(i), entity.getTransportLines().get(j));
          if (isValid == false) {
  ...
  public boolean isInclude(BInvoiceTransportLine line1, BInvoiceTransportLine line2) {
  ...
  private void drawGrid() {
    ...
    inAccountUnitCol.setRendererFactory(rendererFactory);
    ...
    receiverCol.setRendererFactory(rendererFactory);
  ...
  private class GridDataProvider implements RGridDataProvider,
    ...
    public Object getData(int row, int col) {
      ...
      if (col == lineNumberCol.getIndex()) {
        ...
        return rowData.getInAccountUnit();
      } else if (col == receiverCol.getIndex()) {
        return rowData.getReceiver();
  ...
  private class CellRendererFactory implements RCellRendererFactory {
    ...
    public RCellRenderer makeRenderer(RGrid rGrid, RGridColumnDef colDef, int row, int col,
      ...
      if (fieldName.equals(inAccountUnitCol.getName())) {
        return new InAccountUnitRenderer(grid, colDef, row, col, data);
      } else if (fieldName.equals(receiverCol.getName())) {
        return new ReceiverRenderer(grid, colDef, row, col, data);
      }  else if (fieldName.equals(startNumberCol.getName())) {
  ...
  private class InAccountUnitRenderer extends EditGridCellWidgetRenderer {
    private AccountUnitUCNBox field;

    public InAccountUnitRenderer(EditGrid grid, RGridColumnDef colDef, int row, int col, Object data) {
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.setCaption(PInvoiceTransportDef.constants.inAccountUnit());
      field.addKeyDownHandler(new DefaultFocusNextHandler());
      field.addChangeHandler(new ChangeHandler() {

        @Override
        public void onChange(ChangeEvent event) {
          BInvoiceTransportLine value = entity.getTransportLines().get(getRow());
          value.setInAccountUnit(field.getValue());
        }
      });
    ...
    public void setValue(Object value) {
      BInvoiceTransportLine detail = entity.getTransportLines().get(getRow());
      field.setValue(detail.getInAccountUnit());
    }
  }

  private class ReceiverRenderer extends EditGridCellWidgetRenderer {
    private EmployeeUCNBox field;

    public ReceiverRenderer(EditGrid grid, RGridColumnDef colDef, int row, int col, Object data) {
      super(grid, colDef, row, col, data);
    }

    @Override
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      field = new EmployeeUCNBox();
      field.setCaption(PInvoiceTransportDef.constants.receiver());
      field.addKeyDownHandler(new DefaultFocusNextHandler());
      field.addChangeHandler(new ChangeHandler() {
        ...
        public void onChange(ChangeEvent event) {
          BInvoiceTransportLine value = entity.getTransportLines().get(getRow());
          value.setReceiver(field.getValue());
        }
      });
      field.setValidator(new RValidator() {
        @Override
    ...
    public void setValue(Object value) {
      BInvoiceTransportLine detail = entity.getTransportLines().get(getRow());
      field.setValue(detail.getReceiver());
    }
  }
  ...
  private class QuantityRenderer extends EditGridCellWidgetRenderer {
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.setValidator(new RValidator() {
        @Override
        public Message validate(Widget sender, String value) {
          BInvoiceTransportLine line = entity.getTransportLines().get(getRow());
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.addChangeHandler(new ChangeHandler() {
        ...
        public void onChange(ChangeEvent event) {
          ...
          if (field.isValid() && isNumeric(value.getStartNumber())
              && isNumeric(value.getEndNumber())
  ...
  private class Click_Handler implements ClickHandler {
    ...
    public void onClick(ClickEvent event) {
      if (appendAction == event.getSource()) {
        grid.appendValue(entity.getTransportLines().size());
  ...
  @Override
  public void onAction(RActionEvent event) {
    entity.getTransportLines().clear();
    grid.refresh();
    for (int i = 0; i < entity.getTransportLines().size(); i++) {
      Widget widget = grid.getWidget(i, 5);
