 ...
 * 
 */
public class BPaymentLineDefrayalValidator2 implements RValidatable {

  private List<Message> messages = new ArrayList<Message>();
  private BPayment entity;
  private BReceiptConfig config = EPReceipt.getInstance().getConfig();

  public BPaymentLineDefrayalValidator2(BPayment entity) {
    assert entity != null;
  ...
  public boolean validate() {
    ...
    if (CPaymentDefrayalType.line.equals(entity.getDefrayalType())) {
      for (int i = 0; i < entity.getAccountLines().size(); i++) {
        ...
        int tabId;
        tabId = PaymentLineLocator.ACCOUNTMULTITAB_ID;
        ...
        validateLineTotal(line, i);
      }
    }

    // 验证账款明细行中的收付明细
    if (CPaymentDefrayalType.line.equals(entity.getDefrayalType())) {
      for (int i = 0; i < entity.getCollectionLines().size(); i++) {
        BPaymentCollectionLine line = entity.getCollectionLines().get(i);
        int tabId;
        tabId = PaymentLineLocator.COLLECTIONTAB_ID;
        validateCollectionLineCashDefrayal(line, tabId, i);
      }
    }
    ...
    // 验证长短款收款方式总额是否超过长短款配置金额
    validateBillCashDefrayal();
  ...
  private void validateLineCashDefrayal(BPaymentLine line, int tabId, int i, boolean isAccountLine) {
    ...
    for (int j = 0; j < line.getCashs().size(); j++) {
      ...
      if (cash.getPaymentType() != null
        ...
        if (config.getReceiptPaymentType() != null && cash.getPaymentType() != null
            && cash.getTotal() != null
            && ObjectUtil.equals(cash.getPaymentType(), config.getReceiptPaymentType())
            && cash.getTotal().abs().compareTo(config.getReceiptPaymentTypeLimit()) > 0) {
          String msg = ReceiptMessages.M.lineErrorPaymentLimit(caption, (j + 1), config
              .getReceiptPaymentType().toFriendlyStr(), config.getReceiptPaymentTypeLimit()
              .toString());
          Message message = Message.error(msg, locator);
          messages.add(message);
          continue;
        ...
        if (cash.getPaymentType() != null
            ...
            && line.getUnpayedTotal() != null && line.getUnpayedTotal().getTotal() != null
            && line.getUnpayedTotal().getTotal().compareTo(BigDecimal.ZERO) > 0
            && cash.getTotal() != null && cash.getTotal().compareTo(BigDecimal.ZERO) < 0) {
          String msg = ReceiptMessages.M.cannotLessThan2(caption, (j + 1),
   ...
   *          要验证的收付款单明细行是否是账款明细行。
   */
  private void validateCollectionLineCashDefrayal(BPaymentLine line, int tabId, int i) {
    String caption = PPaymentCollectionLineDef.TABLE_CAPTION + CommonMessages.M.lineNumber(i + 1)
        + "/" + PPaymentLineCashDef.TABLE_CAPTION;
    for (int j = 0; j < line.getCashs().size(); j++) {
      BPaymentLineCash cash = line.getCashs().get(j);
      PaymentLineDefrayalLineLocator locator = getCollectionLocator(tabId, i, j,
          BPaymentLineCash.FN_LINECASHTOTAL);
      if (cash.getPaymentType() != null
          && !StringUtil.isNullOrBlank(cash.getPaymentType().getUuid())) {
        if (cash.getTotal() != null
  ...
  private void validateLineDepositDefrayal(BPaymentLine line, int tabId, int i,
    ...
    for (int j = 0; j < line.getDeposits().size(); j++) {
      ...
      if (line.getUnpayedTotal() != null && line.getUnpayedTotal().getTotal() != null
          && line.getUnpayedTotal().getTotal().compareTo(BigDecimal.ZERO) < 0
          && deposit.getTotal() != null && deposit.getTotal().compareTo(BigDecimal.ZERO) != 0) {
        String msg = ReceiptMessages.M.indexError(caption, (j + 1),
            ReceiptMessages.M.lineNoDeposit());
        message = Message.error(msg, locator);
        messages.add(message);
        continue;
      }
      if (deposit.getTotal() != null && deposit.getTotal().compareTo(BigDecimal.ZERO) < 0) {
        ...
        String msg = CommonMessages.M.cannotMoreThan2(caption, (j + 1),
            PPaymentLineDepositDef.constants.total(),
            CommonMessages.M.remainTotal() + deposit.getRemainTotal());
        message = Message.error(msg, locator);
        messages.add(message);
      }
    }
   ...
   * @param line
   */
  private void validateLineTotal(BPaymentLine line, int i) {
    if (line.getUnpayedTotal() != null
        && line.getDefrayalTotal() != null
        && line.getUnpayedTotal().getTotal().compareTo(BigDecimal.ZERO) < 0
        && (line.getDefrayalTotal().compareTo(BigDecimal.ZERO) > 0 || line.getDefrayalTotal()
            .compareTo(line.getUnpayedTotal().getTotal()) < 0)) {
      Message message = Message.error(ReceiptMessages.M.indexError(
          PPaymentAccountLineDef.TABLE_CAPTION, i + 1,
          ReceiptMessages.M.totalBetween(line.getUnpayedTotal().getTotal().toString())));
      messages.add(message);
    }
  }

  /** 验证长短款收款方式总额是否超过长短款配置金额 */
  private void validateBillCashDefrayal() {
    for (int i = 0; i < entity.getCashs().size(); i++) {
      ...
      if (config.getReceiptPaymentType() != null && cash.getPaymentType() != null
        ...
        Message message = Message
            .error(ReceiptMessages.M.lineTotalErrorPaymentLimit(CommonMessages.M.paymentInfo()
                + "/" + PPaymentCashDefrayalDef.constants.tableCaption(), config
                .getReceiptPaymentType().toFriendlyStr(), config.getReceiptPaymentTypeLimit()));
        messages.add(message);
      }
    }
    ...
    for (int i = 0; i < entity.getDeposits().size(); i++) {
      ...
      if (deposit.getSubject() != null && !StringUtil.isNullOrBlank(deposit.getSubject().getUuid())) {
        if (deposit.getTotal().compareTo(deposit.getRemainTotal()) > 0) {
          Message message = Message.error(ReceiptMessages.M.cannotMoreThanRemainTotal(
  ...
  private PaymentLineDefrayalLineLocator getCollectionLocator(int tabId, int lineNumber,
      int lineNumber2, String fieldId) {
    return new PaymentLineDefrayalLineLocator(tabId, lineNumber, lineNumber2, fieldId);
  }
