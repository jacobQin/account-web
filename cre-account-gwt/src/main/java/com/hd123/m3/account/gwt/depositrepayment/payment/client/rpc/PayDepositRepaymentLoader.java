 ...
 * 文件名：	PayDepositRepaymentLoader.java
 * 模块说明：	
 * 修改历史：
 ...
 * 
 */
public class PayDepositRepaymentLoader {
  ...
  public static void decodeParams(JumpParameters params, final Command callback) {
    ...
    if (ep.isProcessMode()) {
      ...
      billUuid = params.getUrlRef().get(PayDepositRepaymentUrlParams.Base.PN_ENTITY_UUID);
      billNumber = params.getUrlRef().get(PayDepositRepaymentUrlParams.Base.PN_ENTITY_BILLNUMBER);
    }
    entity = (BDepositRepayment) params.getExtend().get(EPPayDepositRepayment.OPN_ENTITY);
  ...
  private static void doLoad(String billUuid, final Command callback) {
    RLoadingDialog.show();
    PayDepositRepaymentService.Locator.getService().load(billUuid,
        new RBAsyncCallback2<BDepositRepayment>() {
          @Override
          public void onException(Throwable caught) {
            RLoadingDialog.hide();
            String msg = DepositRepaymentMessage.M
                .cannotFind(PayDepositRepaymentUrlParams.MODULE_CAPTION);
  ...
  private static void doLoadByNumber(final String billNumber, final Command callback) {
    RLoadingDialog.show();
    PayDepositRepaymentService.Locator.getService().loadByNumber(billNumber,
        new RBAsyncCallback2<BDepositRepayment>() {
          @Override
          public void onException(Throwable caught) {
            RLoadingDialog.hide();
          ...
          public void onSuccess(BDepositRepayment result) {
            ...
            if (result != null) {
              ...
              String msg = DepositRepaymentMessage.M.cannotFind2(
                  PayDepositRepaymentUrlParams.MODULE_CAPTION, billNumber);
              RMsgBox.showErrorAndBack(msg, null);
            }
