...
import com.hd123.m3.account.gwt.invoice.payment.client.ui.gadget.PayInvoiceRegGeneralEditGadget;
import com.hd123.m3.account.gwt.invoice.payment.client.ui.gadget.PayInvoiceRegInvoiceEditGadget;
import com.hd123.m3.account.gwt.invoice.payment.client.ui.gadget.PayInvoiceRegInvoiceEditGrid;
import com.hd123.m3.account.gwt.invoice.payment.client.ui.gadget.PayInvoiceRegLineEditGadget;
import com.hd123.m3.account.gwt.invoice.payment.client.ui.gadget.PayInvoiceRegLineEditGrid;
import com.hd123.m3.account.gwt.invoice.payment.intf.client.PayInvoiceRegUrlParams.Edit;
 ...
 * 
 */
public class PayInvoiceRegEditPage extends BaseBpmEditPage implements Edit, RValidatable,
    RActionHandler, HasRActionHandlers, SelectionHandler<Message> {
  private static PayInvoiceRegEditPage instance = null;

  public static PayInvoiceRegEditPage getInstance() {
    if (instance == null) {
      instance = new PayInvoiceRegEditPage();
    }
    return instance;
  }

  public PayInvoiceRegEditPage() {
    super();
    drawToolbar();
    drawSelf();
    afterDraw();
  ...
  private PayInvoiceRegGeneralEditGadget generalGadget;

  private Label invoiceTotal;
  private BillLayoutManager invoiceLayoutMgr;
  private PayInvoiceRegInvoiceEditGrid invoiceGrid;
  private PayInvoiceRegInvoiceEditGadget invoiceGadget;
  ...
  private PayInvoiceRegLineEditGrid lineGrid;
  private PayInvoiceRegLineEditGadget lineGadget;
  /** 从账单导入 */
  ...
  private void drawSelf() {
    ...
    generalGadget = new PayInvoiceRegGeneralEditGadget();
  ...
  private Widget drawLinePanel() {
    ...
    lineGrid = new PayInvoiceRegLineEditGrid();
    lineLayoutMgr.setMainWidget(lineGrid);

    lineGadget = new PayInvoiceRegLineEditGadget();
  ...
  private Widget drawInvoicePanel() {
    ...
    invoiceGrid = new PayInvoiceRegInvoiceEditGrid();
    invoiceLayoutMgr.setMainWidget(invoiceGrid);

    invoiceGadget = new PayInvoiceRegInvoiceEditGadget();
    ...
    addInvoiceAction.setHotKey(new RHotKey(false, false, false, RKey.KEY_CODE_INSERT));
  ...
  private boolean checkIn() {
    if (getEP().isProcessMode())
      return true;
    if (getEP().isPermitted(PayInvoiceRegPermDef.UPDATE) == false) {
  ...
  public void onSelection(SelectionEvent<Message> event) {
    if (event.getSelectedItem().getSource() instanceof InvoiceRegLocator) {
      ...
      if (locator.getLocatorType().equals(InvoiceRegLocator.lineType)) {
        RActionEvent.fire(PayInvoiceRegEditPage.this, InvoiceRegActionName.ONSELECT_LINE,
            Integer.valueOf(locator.getLineNumber() - 1));
      } else if (locator.getLocatorType().equals(InvoiceRegLocator.invoiceType)) {
        RActionEvent.fire(PayInvoiceRegEditPage.this, InvoiceRegActionName.ONSELECT_INVOICE,
            Integer.valueOf(locator.getLineNumber() - 1));
      }
    }
  }
  ...
  public void clearValidResults() {
    ...
    invoiceValidator.clearValidResults();
  }
  ...
  private void doSave() {
    ...
    PayInvoiceRegService.Locator.getService().save(entity, getEP().getProcessCtx(),
        new RBAsyncCallback2<BInvoiceReg>() {
          ...
          public void onSuccess(BInvoiceReg result) {
            RLoadingDialog.hide();
            BInvoiceRegLogger.getInstance().log(InvoiceRegMessage.M.modify(), result);
  ...
  private class Handler_cancelAction implements ClickHandler {
    @Override
    public void onClick(ClickEvent event) {
      RMsgBox.showConfirm(InvoiceRegMessage.M.confirmCancel(InvoiceRegMessage.M.edit()),
    ...
    public void onClick(ClickEvent event) {
      if (event.getSource() == addInvoiceAction) {
        ...
        Scheduler.get().scheduleDeferred(new ScheduledCommand() {
          @Override
          public void execute() {
            RActionEvent.fire(PayInvoiceRegEditPage.this, InvoiceRegActionName.ADD_INVOICE);
          }
        });
      } else if (event.getSource() == insertInvoiceAction) {
        RActionEvent.fire(PayInvoiceRegEditPage.this, InvoiceRegActionName.INSERT_INVOICE);
      } else if (event.getSource() == deleteInvoiceAction) {
        RActionEvent.fire(PayInvoiceRegEditPage.this, InvoiceRegActionName.BATCH_DELETE_INVOICE);
      } else if (event.getSource() == deleteLineAction) {
        RActionEvent.fire(PayInvoiceRegEditPage.this, InvoiceRegActionName.BATCH_DELETE_LINE);
  ...
  private void addAccountFromStatement() {
    if (statementDialog == null) {
      statementDialog = new AccByStatementBrowserDialog(DirectionType.payment.getDirectionValue(),
    ...
    statementDialog.setInvoiceRegUuid(entity.getUuid());
  ...
  private void addAccountFromPaymentNotice() {
    ...
    paymentNoticeDialog.setInvoiceRegUuid(entity.getUuid());
  ...
  public void addAccountFromBill() {
    if (billDialog == null) {
      billDialog = new AccByBillBrowserDialog(DirectionType.payment.getDirectionValue(),
    ...
    billDialog.setInvoiceRegUuid(entity.getUuid());
  ...
  public void addAccount() {
    if (dialog == null) {
      dialog = new AccBrowserDialog(DirectionType.payment.getDirectionValue(),
    ...
    dialog.setInvoiceRegUuid(entity.getUuid());
  ...
  public void refreshToolbar(RToolbar toolbar) {
    saveAction.setVisible(getEP().isPermitted(PayInvoiceRegPermDef.UPDATE));
  }
  ...
  private class ProcessDialog extends M3BaseOutgoingDialog {
    ...
    protected void doExcute(BOperation operation, AsyncCallback<Object> callback) {
      ...
      PayInvoiceRegServiceAgent.executeTask(processCtx, operation, entity, true, this);
    }
  }
  ...
  protected void refreshEntity() {
    ...
    addStatementAction.setEnabled(getEP().isPermitted(PayInvoiceRegPermDef.ADDFROMSTATEMENT));
    addPaymentNoticeAction.setEnabled(getEP().isPermitted(PayInvoiceRegPermDef.ADDFROMPAYNOTICE));
    addAccBillAction.setEnabled(getEP().isPermitted(PayInvoiceRegPermDef.ADDFROMSRCBILL));
    addAccountAction.setEnabled(getEP().isPermitted(PayInvoiceRegPermDef.ADDFROMACCOUNT));
