 ...
 * 
 */
public class InvoiceExchangeInvToInvAccountEditWidget extends RCaptionBox implements RActionHandler {
  ...
  private RGridColumnDef oldInvoiceCodeCol;
  private RGridColumnDef oldInvoiceNumberCol;
  ...
  public void setValue(BInvoiceExchange entity) {
    ...
    entity.setNewInvoiceNumber(null);
  ...
  public InvoiceExchangeInvToInvAccountEditWidget() {
  ...
  private class StockBrowserBoxValueChangeHandler implements ValueChangeHandler<String> {
    ...
    public void onValueChange(ValueChangeEvent<String> event) {
      ...
      if (balanceLinesContainsStock(value)) {
        showWarning(entity.getNewInvoiceNumber(), value.getInvoiceNumber(), invoiceStockBrowserBox);
        return;
      }

      entity.setNewInvoiceNumber(value.getInvoiceNumber());
  ...
  private Widget drawGrid() {
    ...
    oldInvoiceCodeCol = new RGridColumnDef(PInvoiceExchBalanceLineDef.oldInvoiceCode);
    oldInvoiceCodeCol.setWidth("80px");
    grid.addColumnDef(oldInvoiceCodeCol);

    oldInvoiceNumberCol = new RGridColumnDef(PInvoiceExchBalanceLineDef.oldInvoiceNumber);
    oldInvoiceNumberCol.setWidth("100px");
    grid.addColumnDef(oldInvoiceNumberCol);
  ...
  private class GridDataProvider implements RGridDataProvider,
    ...
    public Object getData(int row, int col) {
      ...
      if (col == lineNumberCol.getIndex()) {
        return row + 1;
      } else if (col == oldInvoiceCodeCol.getIndex()) {
        return rowData.getOldCode();
      } else if (col == oldInvoiceNumberCol.getIndex()) {
  ...
  private class InvoiceNumberRenderer extends EditGridCellWidgetRenderer {
    ...
    private class InvoiceStockFieldHandler implements ValueChangeHandler<String>, RValidator {
      ...
      public void onValueChange(ValueChangeEvent<String> event) {
        ...
        if (balanceLinesContainsStock(stock)) {
          value.setNewNumber(field.getValue());
          return;
        }
      ...
      public Message validate(Widget sender, String value) {
        ...
        if (balanceLinesContainsStock(field.getRawValue())) {
          String message = InvoiceExchangeMessages.M.numberExistsInBalanceLines(field.getValue());
  ...
  private void showWarning(final String oldNumber, String newNumber,
      final StartNumberBrowserBox field) {
    RMsgBox.show(InvoiceExchangeMessages.M.numberExistsInBalanceLines(newNumber),
        RMsgBox.ICON_WARN, null, null, new RMsgBox.Callback() {

          @Override
          public void onClosed(ButtonConfig clickedButton) {
            field.setValue(oldNumber);
            field.setFocus(true);
          }
        });
  }
  ...
  /** 红冲明细中是否包含所选择的库存发票 */
  private boolean balanceLinesContainsStock(BInvoiceStock stock) {
  ...
  public void onAction(RActionEvent event) {
    if (ActionName.CHANGE_INVOICEDETAILS_ACTION.equals(event.getActionName())
        && BInvToInvType.exchange.equals(entity.getInvToInvType())) {
      refreshAccounts();
    }
  }
