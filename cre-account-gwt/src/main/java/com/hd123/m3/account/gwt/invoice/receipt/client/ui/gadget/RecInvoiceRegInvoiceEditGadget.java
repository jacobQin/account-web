 ...
 * 
 */
public class RecInvoiceRegInvoiceEditGadget extends Composite implements HasRActionHandlers,
    RActionHandler, HasFocusables {

  public RecInvoiceRegInvoiceEditGadget() {
    drawSelf();
  }

  public final static BigDecimal MAX_TOTAL = BoundaryValue.BIGDECIMAL_MAXVALUE_S2;
  ...
  private void initInvoiceType() {
    Map<String, String> billtypeMap = EPRecInvoiceReg.getInstance().getInvoiceTypeMap();
      ...
      public void onKeyDown(KeyDownEvent event) {
        if (event.getNativeKeyCode() == RKey.KEY_CODE_ENTER) {
          ...
          if (entity.getInvoices().get(entity.getInvoices().size() - 1) == invoice) {
            RActionEvent
                .fire(RecInvoiceRegInvoiceEditGadget.this, InvoiceRegActionName.ADD_INVOICE);
          } else {
            invoiceTypeField.setFocus(true);
            element = GWTUtil.getActiveElement();
            RActionEvent.fire(RecInvoiceRegInvoiceEditGadget.this,
  ...
  private void refreshTotalField() {
    ...
    taxField.setValue(invoice.getTotal() == null ? null : invoice.getTotal().getTax()
        .divide(BigDecimal.ONE, ep.getScale(), ep.getRoundingMode()));
  ...
  private class Handler_prevAction implements ClickHandler {
    ...
    public void onClick(ClickEvent event) {
      element = GWTUtil.getActiveElement();
      GWTUtil.blurActiveElement();
      RActionEvent.fire(RecInvoiceRegInvoiceEditGadget.this, InvoiceRegActionName.PREV_INVOICE);
    }
  }
  ...
  private class Handler_nextAction implements ClickHandler {
    ...
    public void onClick(ClickEvent event) {
      element = GWTUtil.getActiveElement();
      GWTUtil.blurActiveElement();
      RActionEvent.fire(RecInvoiceRegInvoiceEditGadget.this, InvoiceRegActionName.NEXT_INVOICE);
    }
  }
