...
public class DepositDefrayalEditGadget extends RCaptionBox implements RActionHandler,
    ...
    for (int i = values.size() - 1; i >= 0; i--) {
      ...
      for (int j = 0; j < i; j++) {
        ...
        if (deposit.getSubject() != null && d.getSubject() != null
          ...
          String msg = ReceiptMessages.M
              .lineRepeatError(
                  PPaymentDepositDefrayalDef.TABLE_CAPTION + ReceiptMessages.M.lineNumber(i + 1)
                      + ReceiptMessages.M.subjectAndContract(), j + 1);
  ...
  private void refreshDetail() {
    ...
    if (values.isEmpty()) {
      ...
      depositDefrayal.setSubject(EPReceipt.getInstance().getDefaultOption().getPrePaySubject());
