 ...
 * 文件名：	BizRecDepositRepaymentConverter.java
 * 模块说明：	
 * 修改历史：
 ...
 * 
 */
public class BizRecDepositRepaymentConverter extends
    BizAccStandardBillConverter<BDepositRepayment, DepositRepayment> {

  private static BizRecDepositRepaymentConverter instance;

  public static BizRecDepositRepaymentConverter getInstance() {
    if (instance == null)
      instance = new BizRecDepositRepaymentConverter();
    return instance;
  }
  ...
  public DepositRepayment convert(BDepositRepayment source) throws ConversionException {
    ...
    for (BDepositRepaymentLine line : source.getLines()) {
      if (line.getSubject() == null || line.getSubject().getUuid() == null)
        continue;
      lines.add(BizRecDepositRepaymentLineConverter.getInstance().convert(line));
    }
