 ...
 * 
 */
public class RecInvoiceRegLineEditGadget extends Composite implements RActionHandler,
    HasRActionHandlers, HasFocusables {

  public RecInvoiceRegLineEditGadget() {
    drawSelf();
  }
  ...
  private Widget drawRightPanel() {
    ...
    totalField.setMaxValue(BoundaryValue.BIGDECIMAL_MAXVALUE_S2.doubleValue(), false);
    totalField.setEnabled(EPRecInvoiceReg.getInstance().getConfig().getRegTotalWritable());
    ...
    amountField.setEnabled(EPRecInvoiceReg.getInstance().getConfig().getRegTotalWritable());
    amountField.setMaxValue(BoundaryValue.BIGDECIMAL_MAXVALUE_S2.doubleValue());
    ...
    taxField.setEnabled(EPRecInvoiceReg.getInstance().getConfig().getRegTotalWritable());
      ...
      public void onKeyDown(KeyDownEvent event) {
        if (event.getNativeKeyCode() == RKey.KEY_CODE_ENTER) {
          ...
          if (entity.getLines().get(entity.getLines().size() - 1) == line) {
            RActionEvent.fire(RecInvoiceRegLineEditGadget.this, InvoiceRegActionName.ADD_INVOICE,
                "");
          } else {
            remarkField.setFocus(true);
            element = GWTUtil.getActiveElement();
            RActionEvent.fire(RecInvoiceRegLineEditGadget.this, InvoiceRegActionName.NEXT_INVOICE);
          }
        }
      }
    });
  ...
  private void refresh() {
    ...
    if (line.getAcc1().getSourceBill() != null
      ...
      BBillType billType = EPRecInvoiceReg.getInstance().getBillTypeMap()
    ...
    RActionEvent.fire(RecInvoiceRegLineEditGadget.this, InvoiceRegActionName.CHANGE_LINE_VALUE);
  }
  ...
  private class Handler_prevAction implements ClickHandler {
    ...
    public void onClick(ClickEvent event) {
      element = GWTUtil.getActiveElement();
      GWTUtil.blurActiveElement();
      RActionEvent.fire(RecInvoiceRegLineEditGadget.this, InvoiceRegActionName.PREV_LINE);
    }
  }
  ...
  private class Handler_nextAction implements ClickHandler {
    ...
    public void onClick(ClickEvent event) {
      element = GWTUtil.getActiveElement();
      GWTUtil.blurActiveElement();
      RActionEvent.fire(RecInvoiceRegLineEditGadget.this, InvoiceRegActionName.NEXT_LINE);
    }
  }
  ...
  private class Handler_deleteAction implements ClickHandler {
    @Override
    public void onClick(ClickEvent event) {
      RActionEvent.fire(RecInvoiceRegLineEditGadget.this, InvoiceRegActionName.DELETE_CURRENT_LINE);
    }
  }
