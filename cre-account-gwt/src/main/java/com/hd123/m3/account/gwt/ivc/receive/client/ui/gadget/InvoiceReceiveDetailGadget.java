 ...
 * 文件名：	InvoiceReceiveDetailGadget.java
 * 模块说明：	
 * 修改历史：
 ...
 * 
 */
public class InvoiceReceiveDetailGadget extends RCaptionBox implements RActionHandler {
  ...
  public InvoiceReceiveDetailGadget() {
    super();
  ...
  private boolean validateContain() {
    ...
    boolean result = true;
    for (int i = 0; i < entity.getReceiveLines().size(); i++) {
      for (int j = i + 1; j < entity.getReceiveLines().size(); j++) {
        if ((!StringUtil.isNullOrBlank(entity.getReceiveLines().get(i).getStartNumber()))
            && (!StringUtil.isNullOrBlank(entity.getReceiveLines().get(i).getEndNumber()))
            && (!StringUtil.isNullOrBlank(entity.getReceiveLines().get(j).getStartNumber()))
            && (!StringUtil.isNullOrBlank(entity.getReceiveLines().get(j).getEndNumber()))
            && isNumeric(entity.getReceiveLines().get(i).getStartNumber())
            && isNumeric(entity.getReceiveLines().get(i).getEndNumber())
            && isNumeric(entity.getReceiveLines().get(j).getStartNumber())
            && isNumeric(entity.getReceiveLines().get(j).getEndNumber())) {
          isValid = !isInclude(entity.getReceiveLines().get(i), entity.getReceiveLines().get(j));
          if (isValid == false) {
  ...
  private class StartNumberRenderer extends EditGridCellWidgetRenderer {
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.addChangeHandler(new ChangeHandler() {
        ...
        public void onChange(ChangeEvent event) {
          BInvoiceLine value = entity.getReceiveLines().get(getRow());
    ...
    public void setValue(Object value) {
      BInvoiceLine detail = entity.getReceiveLines().get(getRow());
  ...
  private class QuantityRenderer extends EditGridCellWidgetRenderer {
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.setValidator(new RValidator() {
        @Override
        public Message validate(Widget sender, String value) {
          BInvoiceLine line = entity.getReceiveLines().get(getRow());
  ...
  private class Click_Handler implements ClickHandler {
    ...
    public void onClick(ClickEvent event) {
      if (appendAction == event.getSource()) {
        grid.appendValue(entity.getReceiveLines().size());
