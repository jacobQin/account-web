 ...
 * 文件名：	InvoiceRecycleDetailGadget.java
 * 模块说明：	
 * 修改历史：
 ...
 *
 */
public class InvoiceRecycleDetailGadget extends RCaptionBox implements RActionHandler {
  ...
  public InvoiceRecycleDetailGadget() {
    super();
  ...
  private boolean validateContain() {
    ...
    for (int i = 0; i < entity.getLines().size(); i++) {
      for (int j = i + 1; j < entity.getLines().size(); j++) {
        if ((!StringUtil.isNullOrBlank(entity.getLines().get(i).getStartNumber()))
            && (!StringUtil.isNullOrBlank(entity.getLines().get(i).getEndNumber()))
            && (!StringUtil.isNullOrBlank(entity.getLines().get(j).getStartNumber()))
            && (!StringUtil.isNullOrBlank(entity.getLines().get(j).getEndNumber()))
            && isNumeric(entity.getLines().get(i).getStartNumber())
            && isNumeric(entity.getLines().get(i).getEndNumber())
            && isNumeric(entity.getLines().get(j).getStartNumber())
            && isNumeric(entity.getLines().get(j).getEndNumber())) {
          isValid = !isInclude(entity.getLines().get(i), entity.getLines().get(j));
          if (isValid == false) {
  ...
  public boolean isInclude(BInvoiceRecycleLine line1, BInvoiceRecycleLine line2) {
  ...
  private void drawGrid() {
    ...
    realQtyCol.setRendererFactory(rendererFactory);
  ...
  private class CellRendererFactory implements RCellRendererFactory {
    ...
    public RCellRenderer makeRenderer(RGrid rGrid, RGridColumnDef colDef, int row, int col,
      ...
      } else if (fieldName.equals(realQtyCol.getName())) {
        return new RealQtyRenderer(grid, colDef, row, col, data);
  ...
  private class StartNumberRenderer extends EditGridCellWidgetRenderer {
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.setStates(BStockState.used.name(), BStockState.aborted.name());
      field.addChangeHandler(new ChangeHandler() {
        @Override
        public void onChange(ChangeEvent event) {
          BInvoiceRecycleLine value = entity.getLines().get(getRow());
    ...
    public void setValue(Object value) {
      BInvoiceRecycleLine detail = entity.getLines().get(getRow());
  ...
  private class QuantityRenderer extends EditGridCellWidgetRenderer {
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.setValidator(new RValidator() {
        @Override
        public Message validate(Widget sender, String value) {
          BInvoiceRecycleLine line = entity.getLines().get(getRow());
      ...
      field.addChangeHandler(new ChangeHandler() {
        ...
        public void onChange(ChangeEvent event) {
          ...
          if (field.isValid() && isNumeric(value.getStartNumber())
            ...
            value.setRealQty(value.getQuantity());
            value.setBalanceQty(0);
            grid.refresh();
          }
        }
  ...
  private class RealQtyRenderer extends EditGridCellWidgetRenderer {
    private RNumberBox<Integer> field;

    public RealQtyRenderer(EditGrid grid, RGridColumnDef colDef, int row, int col, Object data) {
      super(grid, colDef, row, col, data);
    }
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.addChangeHandler(new ChangeHandler() {
        ...
        public void onChange(ChangeEvent event) {
          ...
          if (field.getValue() == null) {
            value.setRealQty(0);
            value.setBalanceQty(value.getQuantity() - value.getRealQty());
            grid.refresh();
            return;
          }
          if (field.getValueAsBigDecimal().compareTo(new BigDecimal(Integer.MAX_VALUE)) < 0) {
      ...
      field.setValidator(new RValidator() {
        ...
        public Message validate(Widget sender, String value) {
          BInvoiceRecycleLine line = entity.getLines().get(getRow());
          if (checkEmpty && field.getValueAsBigDecimal().compareTo(BigDecimal.ZERO) < 0) {
            return Message.error(field.getCaption() + ":" + InvoiceCommonMessages.M.realNeedZero(),
                field);
          }
          if (field.getValueAsBigDecimal().compareTo(new BigDecimal(Integer.MAX_VALUE)) > 0) {
            return Message.error(field.getCaption() + ":"
          ...
          if (checkEmpty && field.getValue() != null
              && field.getValueAsInteger() > line.getQuantity()) {
            return Message.error(field.getCaption() + ":"
                + InvoiceCommonMessages.M.maxNotQty(line.getQuantity() + ""));
          }
          return null;
        }
      });
    ...
    public void setValue(Object value) {
      BInvoiceRecycleLine detail = entity.getLines().get(getRow());
      field.setValue(detail.getRealQty());
    }
  }
