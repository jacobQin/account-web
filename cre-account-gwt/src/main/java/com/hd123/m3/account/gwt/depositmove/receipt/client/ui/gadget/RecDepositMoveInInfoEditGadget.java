 ...
 * 文件名：	RecDepositMoveInInfoEditGadget.java
 * 模块说明：	
 * 修改历史：
 ...
 * 
 */
public class RecDepositMoveInInfoEditGadget extends RCaptionBox implements HasRActionHandlers,
    RActionHandler {
  private EPRecDepositMove ep = EPRecDepositMove.getInstance();

  public RecDepositMoveInInfoEditGadget() {
    drawSelf();
  }
  ...
  private void doRefreshContract() {
    ...
    RecDepositMoveService.Locator.getService().getUniqueContract(entity.getAccountUnit().getUuid(),
        entity.getInCounterpart().getUuid(), entity.getInCounterpart().getCounterpartType(), false,
        new RBAsyncCallback2<BAdvanceContract>() {
          @Override
  ...
  private void refreshCounterpart(BAdvanceContract contract) {
    ...
    counterpartUCNBox.setValue(entity.getInCounterpart());
    if (ep.getCounterpartTypeMap().size() > 1) {
      String counterpartType = entity.getInCounterpart() == null ? null : entity.getInCounterpart()
          .getCounterpartType();
      countpartTypeField.setValue(counterpartType == null ? null : ep.getCounterpartTypeMap().get(
          counterpartType));
    }
