 ...
 * 
 */
public class DepositDefrayalLineEditGadget extends Composite implements HasRActionHandlers,
    RValidatable, HasFocusables, Focusable {

  public static final String WIDTH_DETAIL_COL0 = "300px";
  ...
  public DepositDefrayalLineEditGadget() {
    ...
    addButton = new RToolbarButton(RActionFacade.ADD_ITEM, new ClickHandler() {
      ...
      public void onClick(ClickEvent arg0) {
        ...
        RActionEvent.fire(DepositDefrayalLineEditGadget.this,
            ActionName.ACTION_DEPOSITDEFRAYALLINE_ADDDETAIL, value.getLineNumber());
      }
    });
    ...
    removeButton = new RToolbarButton(RActionFacade.REMOVE_ITEM, new ClickHandler() {
      ...
      public void onClick(ClickEvent arg0) {
        GWTUtil.blurActiveElement();
        RActionEvent.fire(DepositDefrayalLineEditGadget.this,
            ActionName.ACTION_DEPOSITDEFRAYALLINE_REMOVEDETAIL, value.getLineNumber());
      }
    });
    ...
    public void onValueChange(ValueChangeEvent event) {
      ...
      if (event.getSource() == subjectField) {
        ...
        if (contractField.getValue() == null) {
          contractField.setValue(contractField.getOptions().getValue(0));
  ...
  private void validateTotalField() {
    if (value.getTotal() != null && value.getTotal().compareTo(BigDecimal.ZERO) < 0) {
      totalField.addErrorMessage(PaymentMessages.M.cannotLessThan(
          PPaymentDepositDefrayalDef.constants.total(), "0"));
    } else if (value.getTotal() != null && value.getTotal().compareTo(value.getRemainTotal()) > 0) {
      totalField.addErrorMessage(PaymentMessages.M.cannotMoreThan(
          PPaymentDepositDefrayalDef.constants.total(), PaymentMessages.M.remainTotal()));
    } else {
      totalField.clearValidResults();
    }
  }
  ...
  private void setDefaultSubject() {
    if (subjectField.getOptions().getByValue(
        EPPayment.getInstance().getDefaultOption().getPrePaySubject()) != null) {
      value.setSubject(EPPayment.getInstance().getDefaultOption().getPrePaySubject());
    } else {
      if (subjectField.getOptions().size() > 0) {
        value.setSubject(subjectField.getOptions().getValue(0));
      } else {
