 ...
 * 
 */
public class BReceiptDefrayalApportionHelper {
  BReceiptConfig config = EPReceipt.getInstance().getConfig();

  public void apportion(BPayment payment, int scale, RoundingMode roundingMode) {
    if (CPaymentDefrayalType.bill.equals(payment.getDefrayalType()) == false) {
      return;
    }
    ...
    for (int i = 0; i < payment.getCashs().size(); i++) {
      cashs.put(i, payment.getCashs().get(i).getTotal());
    }
    ...
    for (int i = 0; i < payment.getCashs().size(); i++) {
      BPaymentCashDefrayal defrayal = payment.getCashs().get(i);
  ...
  public void apportionAccountLines(BPayment payment, int scale, RoundingMode roundingMode) {
    ...
    for (BPaymentAccountLine line : payment.getAccountLines()) {
      line.getTotal().setTax(
          BTaxCalculator.tax(line.getTotal().getTotal(), line.getAcc1().getTaxRate(), scale,
              roundingMode));
    }
    // 根据账款实收重算滞纳金
    BOverdueCalcHelper.calculate(payment, scale, roundingMode, EPReceipt.getInstance()
        .getOverdueDefault());
  }
  ...
  private boolean needApportion(BPayment payment, BigDecimal unpayedTotal) {
    ...
    for (BPaymentCashDefrayal defrayal : payment.getCashs()) {
  ...
  private void apportionCash(BPayment payment, boolean negative, boolean apportionCollection) {
    for (BPaymentCashDefrayal cashDefrayal : payment.getCashs()) {
      BigDecimal leftTotal = cashDefrayal.getTotal();
      BUCN paymentType = cashDefrayal.getPaymentType();
      ...
      for (BPaymentLine line : lines) {
        ...
        lineDefrayal.setLineUuid(line.getUuid());
        line.getCashs().add(lineDefrayal);
        if (undefrayalTotal.abs().compareTo(leftTotal.abs()) < 0) {
          line.setDefrayalTotal(line.getUnpayedTotal().getTotal());
          leftTotal = leftTotal.subtract(undefrayalTotal);
          lineDefrayal.setTotal(undefrayalTotal);
