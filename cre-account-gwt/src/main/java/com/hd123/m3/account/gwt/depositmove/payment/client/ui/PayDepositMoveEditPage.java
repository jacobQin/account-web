 ...
 * 文件名：	PayDepositMoveEditPage.java
 * 模块说明：	
 * 修改历史：
...
import com.hd123.m3.account.gwt.depositmove.payment.intf.client.PayDepositMoveUrlParams.Edit;
 ...
 * 
 */
public class PayDepositMoveEditPage extends BaseBpmEditPage implements Edit, RValidatable {
  private static PayDepositMoveEditPage instance = null;

  public static PayDepositMoveEditPage getInstance() {
    if (instance == null)
      instance = new PayDepositMoveEditPage();
    return instance;
  }

  public PayDepositMoveEditPage() {
    super();
    drawToolbar();
    drawSelf();
  }
  ...
  private boolean checkIn() {
    if (getEP().isProcessMode())
      return true;
    if (getEP().isPermitted(PayDepositMovePermDef.UPDATE) == false) {
  ...
  public void refreshToolbar(RToolbar toolbar) {
    saveAction.setVisible(getEP().isPermitted(PayDepositMovePermDef.UPDATE));
  }
