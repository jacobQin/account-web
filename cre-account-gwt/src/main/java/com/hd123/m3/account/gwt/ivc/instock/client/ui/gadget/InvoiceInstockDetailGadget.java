 ...
 * 文件名：	InvoiceInstockDetailGadget.java
 * 模块说明：	
 * 修改历史：
 ...
 * 
 */
public class InvoiceInstockDetailGadget extends RCaptionBox {
  ...
  public void setValue(BInvoiceInstock entity) {
    this.entity = entity;
    if (entity.getInstockLines() == null) {
      entity.setInstockLines(new ArrayList<BInvoiceLine>());
    }
    grid.setValues(entity.getInstockLines());

    totalCount.setHTML(InvoiceCommonMessages.M.totalCount(entity.getInstockLines().size()));

    grid.refresh();
  }

  public InvoiceInstockDetailGadget() {
    super();
  ...
  private boolean validateContain() {
    ...
    boolean result = true;
    for (int i = 0; i < entity.getInstockLines().size(); i++) {
      for (int j = i + 1; j < entity.getInstockLines().size(); j++) {
        if ((!StringUtil.isNullOrBlank(entity.getInstockLines().get(i).getStartNumber()))
            && (!StringUtil.isNullOrBlank(entity.getInstockLines().get(i).getEndNumber()))
            && (!StringUtil.isNullOrBlank(entity.getInstockLines().get(j).getStartNumber()))
            && (!StringUtil.isNullOrBlank(entity.getInstockLines().get(j).getEndNumber()))
            && isNumeric(entity.getInstockLines().get(i).getStartNumber())
            && isNumeric(entity.getInstockLines().get(i).getEndNumber())
            && isNumeric(entity.getInstockLines().get(j).getStartNumber())
            && isNumeric(entity.getInstockLines().get(j).getEndNumber())) {
          isValid = !isInclude(entity.getInstockLines().get(i), entity.getInstockLines().get(j));
          if (isValid == false) {
  ...
  private void drawGrid() {
    ...
    grid.setCustomSummaryProvider(new RGridSummaryProvider() {
      ...
      public String getSummary(int col) {
        ...
        if (col == quantityCol.getIndex()) {
          int total = 0;
          for (BInvoiceLine line : entity.getInstockLines()) {
    ...
    quantityCol = new RGridColumnDef(PInvoiceLineDef.quantity);
  ...
  private class GridDataProvider implements RGridDataProvider, EditGridDataProduce<BInvoiceLine> {
    ...
    public int getRowCount() {
      return entity == null || entity.getInstockLines() == null ? 0 : entity.getInstockLines()
          .size();
    }

    @Override
    public Object getData(int row, int col) {
      BInvoiceLine rowData = entity.getInstockLines().get(row);
  ...
  private class CellRendererFactory implements RCellRendererFactory {
    ...
    public RCellRenderer makeRenderer(RGrid rGrid, RGridColumnDef colDef, int row, int col,
      ...
      BInvoiceLine value = entity.getInstockLines().get(row);

      if (fieldName.equals(invoiceCodeCol.getName())) {
        return new InvoiceCodeRenderer(grid, colDef, row, col, value);
  ...
  private class InvoiceCodeRenderer extends EditGridCellWidgetRenderer {
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.setWidth("100%");
      field.setRegex("([0-9]|[a-z]|[A-Z])*");
      field.setRegexMessage(InvoiceCommonMessages.M.regexNumOrStrMessage());
      ...
      field.addChangeHandler(new ChangeHandler() {
        ...
        public void onChange(ChangeEvent event) {
          BInvoiceLine value = entity.getInstockLines().get(getRow());
    ...
    public void setValue(Object value) {
      BInvoiceLine detail = entity.getInstockLines().get(getRow());
  ...
  private class QuantityRenderer extends EditGridCellWidgetRenderer {
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.setValidator(new RValidator() {
        @Override
        public Message validate(Widget sender, String value) {
          BInvoiceLine line = entity.getInstockLines().get(getRow());
      ...
      field.addChangeHandler(new ChangeHandler() {
        ...
        public void onChange(ChangeEvent event) {
          ...
          if (field.isValid() && isNumeric(value.getStartNumber())
            ...
            BigInteger end = new BigInteger(value.getEndNumber());
            BigInteger start = new BigInteger(value.getStartNumber());
            value.setQuantity((end.subtract(start).add(BigInteger.ONE)).intValue());
            grid.refresh();
          }
        }
      });
  ...
  private class Click_Handler implements ClickHandler {
    ...
    public void onClick(ClickEvent event) {
      if (appendAction == event.getSource()) {
        grid.appendValue(entity.getInstockLines().size());
