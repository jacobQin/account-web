 ...
 * 文件名：	RecDepositMoveServiceServlet.java
 * 模块说明：	
 * 修改历史：
 ...
 * 
 */
public class RecDepositMoveServiceServlet extends AccBPMServiceServlet implements
    RecDepositMoveService {
  private static final long serialVersionUID = -7798294975847217664L;
  ...
  public BDepositMove create() throws ClientBizException {
    try {
      DepositMove source = getDepositMoveService().create(Direction.RECEIPT);
  ...
  public BDepositMove save(BDepositMove entity, BProcessContext task) throws ClientBizException {
    try {
      ...
      if (entity.getUuid() == null)
        target = getDepositMoveService().create(Direction.RECEIPT);
      else
  ...
  private FlecsQueryDefinition buildQueryDefinition(List<Condition> conditions,
    ...
    queryDef.getFlecsConditions().addAll(
        RecDepositMoveQueryBuilder.getInstance().buildFlecsConditions(conditions));
    queryDef.setOrders(RecDepositMoveQueryBuilder.getInstance().buildQueryOrders(sortOrders));
    return queryDef;
  }
  ...
  private FlecsQueryDefinition buildDefaultQueryDefinition() {
    ...
    queryDef.addCondition(DepositMoves.CONDITION_DIRECTION_EQUALS, Direction.RECEIPT);
  ...
  public interface R {
    ...
    @DefaultStringValue("预存款转移单")
    String moduleCaption();

    @DefaultStringValue("预存款转移单：{0},合同编号：{1}")
    String billCaption();
    ...
    @DefaultStringValue("指定的\"预存款转移单\"不存在。")
    String nullObject();
  }
