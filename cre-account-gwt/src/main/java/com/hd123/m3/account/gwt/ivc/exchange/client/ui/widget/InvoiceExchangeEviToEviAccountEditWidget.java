 ...
 * 
 */
public class InvoiceExchangeEviToEviAccountEditWidget extends RCaptionBox implements RActionHandler {
  ...
  private RGridColumnDef newReceiptCodeCol;
  private RGridColumnDef newReceiptNumberCol;
  ...
  public void setValue(BInvoiceExchange entity) {
    ...
    invoiceStockBrowserBox.setSort(CommonConstants.SORT_RECEIPT);
  ...
  public InvoiceExchangeEviToEviAccountEditWidget() {
  ...
  private Widget drawNumberBrowserBox() {
    ...
    invoiceStockBrowserBox = new StartNumberBrowserBox(InvoiceExchangeMessages.M.receiptNumber(),
  ...
  private Widget drawGrid() {
    ...
    newReceiptCodeCol = new RGridColumnDef(PInvoiceExchBalanceLineDef.newReceiptCode);
    newReceiptCodeCol.setWidth("80px");
    grid.addColumnDef(newReceiptCodeCol);

    newReceiptNumberCol = new RGridColumnDef(PInvoiceExchBalanceLineDef.newReceiptNumber);
    newReceiptNumberCol.setRendererFactory(rendererFactory);
    newReceiptNumberCol.setWidth("100px");
    grid.addColumnDef(newReceiptNumberCol);
  ...
  private class GridDataProvider implements RGridDataProvider,
    ...
    public Object getData(int row, int col) {
      ...
      } else if (col == newReceiptCodeCol.getIndex()) {
        return rowData.getNewCode();
      } else if (col == newReceiptNumberCol.getIndex()) {
  ...
  private class CellRendererFactory implements RCellRendererFactory {
    ...
    public RCellRenderer makeRenderer(RGrid rGrid, RGridColumnDef colDef, int row, int col,
        Object data) {
      if (colDef.getName().equals(newReceiptNumberCol.getName())) {
      ...
      } else if (colDef.getName().endsWith(sourceBillCol.getName())) {
  ...
  private class InvoiceNumberRenderer extends EditGridCellWidgetRenderer {
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field = new StartNumberBrowserBox(InvoiceExchangeMessages.M.newReceiptNumber(),
          BStockState.received.name());
      field.setWidth("100%");
  ...
  public void onAction(RActionEvent event) {
    if (ActionName.CHANGE_INVOICEDETAILS_ACTION.equals(event.getActionName())
        && BExchangeType.eviToEvi.equals(entity.getType())) {
      refreshAccounts();
    }
  }
