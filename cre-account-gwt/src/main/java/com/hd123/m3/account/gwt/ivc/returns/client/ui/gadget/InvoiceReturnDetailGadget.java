 ...
 * 文件名：	InvoiceReturnDetailGadget.java
 * 模块说明：	
 * 修改历史：
 ...
 * 
 */
public class InvoiceReturnDetailGadget extends RCaptionBox implements RActionHandler {
  ...
  public void setValue(BInvoiceReturn entity) {
    this.entity = entity;
    if (entity.getReturnLines() == null) {
      entity.setReturnLines(new ArrayList<BInvoiceLine>());
    }
    grid.setValues(entity.getReturnLines());

    totalCount.setHTML(InvoiceCommonMessages.M.totalCount(entity.getReturnLines().size()));

    grid.refresh();
  }

  public InvoiceReturnDetailGadget() {
    super();
  ...
  private boolean validateContain() {
    ...
    boolean result = true;
    for (int i = 0; i < entity.getReturnLines().size(); i++) {
      for (int j = i + 1; j < entity.getReturnLines().size(); j++) {
        if ((!StringUtil.isNullOrBlank(entity.getReturnLines().get(i).getStartNumber()))
            && (!StringUtil.isNullOrBlank(entity.getReturnLines().get(i).getEndNumber()))
            && (!StringUtil.isNullOrBlank(entity.getReturnLines().get(j).getStartNumber()))
            && (!StringUtil.isNullOrBlank(entity.getReturnLines().get(j).getEndNumber()))
            && isNumeric(entity.getReturnLines().get(i).getStartNumber())
            && isNumeric(entity.getReturnLines().get(i).getEndNumber())
            && isNumeric(entity.getReturnLines().get(j).getStartNumber())
            && isNumeric(entity.getReturnLines().get(j).getEndNumber())) {
          isValid = !isInclude(entity.getReturnLines().get(i), entity.getReturnLines().get(j));
          if (isValid == false) {
  ...
  private void drawGrid() {
    ...
    quantityCol = new RGridColumnDef(PInvoiceLineDef.return_quantity);
  ...
  private class GridDataProvider implements RGridDataProvider, EditGridDataProduce<BInvoiceLine> {
    ...
    public int getRowCount() {
      return entity == null || entity.getReturnLines() == null ? 0 : entity.getReturnLines().size();
    }

    @Override
    public Object getData(int row, int col) {
      BInvoiceLine rowData = entity.getReturnLines().get(row);
  ...
  private class StartNumberRenderer extends EditGridCellWidgetRenderer {
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.addChangeHandler(new ChangeHandler() {
        ...
        public void onChange(ChangeEvent event) {
          BInvoiceLine value = entity.getReturnLines().get(getRow());
    ...
    public void setValue(Object value) {
      BInvoiceLine detail = entity.getReturnLines().get(getRow());
  ...
  private class QuantityRenderer extends EditGridCellWidgetRenderer {
    ...
    protected Widget drawField(RGridColumnDef colDef, Object data) {
      ...
      field.setValidator(new RValidator() {
        @Override
        public Message validate(Widget sender, String value) {
          BInvoiceLine line = entity.getReturnLines().get(getRow());
  ...
  private class Click_Handler implements ClickHandler {
    ...
    public void onClick(ClickEvent event) {
      if (appendAction == event.getSource()) {
        grid.appendValue(entity.getReturnLines().size());
