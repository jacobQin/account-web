...
import com.hd123.m3.account.gwt.invoice.payment.client.ui.gadget.PayInvoiceRegGeneralCreateGadget;
...
import com.hd123.m3.account.gwt.invoice.payment.intf.client.PayInvoiceRegUrlParams.Create;
 ...
 * 
 */
public class PayInvoiceRegCreatePage extends BaseBpmCreatePage implements Create, RValidatable,
    RActionHandler, HasRActionHandlers, SelectionHandler<Message> {
  private static PayInvoiceRegCreatePage instance = null;

  public static PayInvoiceRegCreatePage getInstance() {
    if (instance == null) {
      instance = new PayInvoiceRegCreatePage();
    }
    return instance;
  }

  public PayInvoiceRegCreatePage() {
    super();
    drawToolbar();
    drawSelf();
    afterDraw();
  ...
  private PayInvoiceRegGeneralCreateGadget generalGadget;
  ...
  private void drawSelf() {
    ...
    generalGadget = new PayInvoiceRegGeneralCreateGadget();
  ...
  private boolean checkIn() {
    if (getEP().isBpmMode())
      return true;
    if (getEP().isPermitted(PayInvoiceRegPermDef.CREATE) == false) {
  ...
  private void doCreate(JumpParameters params, final Command callback) {
    String statementUuid = params.getUrlRef().get(PayInvoiceRegUrlParams.Create.PN_STATEMENT_UUID);
    String paymentUuid = params.getUrlRef().get(PayInvoiceRegUrlParams.Create.PN_PAYMENT_UUID);
  ...
  private void doCreate(final Command callback) {
    PayInvoiceRegService.Locator.getService().create(new RBAsyncCallback2<BInvoiceReg>() {
      @Override
  ...
  private void doCreateByStatement(String statementUuid, final Command callback) {
    PayInvoiceRegService.Locator.getService().createByStatement(statementUuid,
        DirectionType.payment.getDirectionValue(), new RBAsyncCallback2<BInvoiceReg>() {
          @Override
          public void onException(Throwable caught) {
            RLoadingDialog.hide();
            String msg = InvoiceRegMessage.M.actionFailed(InvoiceRegMessage.M.create(), getEP()
  ...
  private void doCreateByPayment(String paymentUuid, final Command callback) {
    PayInvoiceRegService.Locator.getService().createByPayment(paymentUuid,
  ...
  public void onSelection(SelectionEvent<Message> event) {
    if (event.getSelectedItem().getSource() instanceof InvoiceRegLocator) {
      ...
      if (locator.getLocatorType().equals(InvoiceRegLocator.lineType)) {
        RActionEvent.fire(PayInvoiceRegCreatePage.this, InvoiceRegActionName.ONSELECT_LINE,
            Integer.valueOf(locator.getLineNumber()));
      } else if (locator.getLocatorType().equals(InvoiceRegLocator.invoiceType)) {
        RActionEvent.fire(PayInvoiceRegCreatePage.this, InvoiceRegActionName.ONSELECT_INVOICE,
            Integer.valueOf(locator.getLineNumber()));
      }
    }
  }
    ...
    public void onClick(ClickEvent event) {
      if (event.getSource() == addInvoiceAction) {
        ...
        Scheduler.get().scheduleDeferred(new ScheduledCommand() {
          @Override
          public void execute() {
            RActionEvent.fire(PayInvoiceRegCreatePage.this, InvoiceRegActionName.ADD_INVOICE);
          }
        });
      } else if (event.getSource() == insertInvoiceAction) {
        RActionEvent.fire(PayInvoiceRegCreatePage.this, InvoiceRegActionName.INSERT_INVOICE);
      } else if (event.getSource() == deleteInvoiceAction) {
        RActionEvent.fire(PayInvoiceRegCreatePage.this, InvoiceRegActionName.BATCH_DELETE_INVOICE);
      } else if (event.getSource() == deleteLineAction) {
        RActionEvent.fire(PayInvoiceRegCreatePage.this, InvoiceRegActionName.BATCH_DELETE_LINE);
