...
public class RecDepositLoader {
  ...
  public static void decodeParams(JumpParameters params, final Command callback) {
    ...
    if (ep.isProcessMode()) {
      ...
      billUuid = params.getUrlRef().get(RecDepositUrlParams.Base.PN_UUID);
      billNumber = params.getUrlRef().get(RecDepositUrlParams.Base.PN_BILLNUMBER);
    }
    entity = (BDeposit) params.getExtend().get(EPRecDeposit.OPN_ENTITY);
  ...
  private static void doLoad(String billUuid, final Command callback) {
    RLoadingDialog.show();
    RecDepositService.Locator.getService().load(billUuid, new RBAsyncCallback2<BDeposit>() {
      @Override
      public void onException(Throwable caught) {
        RLoadingDialog.hide();
        String msg = DepositMessage.M.cannotFind(RecDepositUrlParams.MODULE_CAPTION);
  ...
  private static void doLoadByNumber(final String billNumber, final Command callback) {
    RLoadingDialog.show();
    RecDepositService.Locator.getService().loadByNumber(billNumber,
        new RBAsyncCallback2<BDeposit>() {
          ...
          public void onSuccess(BDeposit result) {
            ...
            if (result != null) {
              ...
              String msg = DepositMessage.M.cannotFind2(RecDepositUrlParams.MODULE_CAPTION,
                  billNumber);
              RMsgBox.showErrorAndBack(msg, null);
            }
          }
