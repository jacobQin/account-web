 ...
 * 文件名：	PayDepositRepaymentGeneralCreateGadget.java
 * 模块说明：	
 * 修改历史：
 ...
 * 
 */
public class PayDepositRepaymentGeneralCreateGadget extends RCaptionBox implements
    HasRActionHandlers, RActionHandler {

  public PayDepositRepaymentGeneralCreateGadget() {
    drawSelf();
  }
  ...
  public void refresh(BDepositRepayment entity) {
    ...
    if (entity.getCounterpart() != null) {
      ...
      if (ep.getCounterpartTypeMap().size() > 1) {
        String counterpartType = entity.getCounterpart().getCounterpartType();
  ...
  private Widget drawGeneralPanel() {
    ...
    countpartField = new RCombinedField() {
      {
        setCaption(EPPayDepositRepayment.getInstance().getFieldCaption(GRes.FIELDNAME_COUNTERPART,
            GRes.R.counterpart()));
        if (ep.getCounterpartTypeMap().size() > 1) {
  ...
  private void onChangeAccountUnit() {
    ...
    RMsgBox.showConfirm(msg, new ConfirmCallback() {
      ...
      public void onClosed(boolean confirmed) {
        ...
        doFireActionEvent();
        RActionEvent.fire(PayDepositRepaymentGeneralCreateGadget.this,
    ...
    RMsgBox.showConfirm(msg, new ConfirmCallback() {
      ...
      public void onClosed(boolean confirmed) {
        if (confirmed == false) {
          counterpartUCNBox.setValue(entity.getCounterpart());
          if (ep.getCounterpartTypeMap().size() > 1)
            countpartTypeField.setValue(entity.getCounterpart().getCounterpartType());
          return;
        }
        entity.setCounterpart(counterpartUCNBox.getRawValue());
        if (ep.getCounterpartTypeMap().size() > 1) {
  ...
  private void doRefreshContract() {
    ...
    PayDepositRepaymentService.Locator.getService().getUniqueAdvance(
