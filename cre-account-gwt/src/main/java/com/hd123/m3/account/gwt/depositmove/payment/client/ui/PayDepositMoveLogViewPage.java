 ...
 * 文件名：	PayDepositMoveLogViewPage.java
 * 模块说明：	
 * 修改历史：
...
import com.hd123.m3.account.gwt.depositmove.payment.intf.client.PayDepositMoveUrlParams.Log;
 ...
 * 
 */
public class PayDepositMoveLogViewPage extends BaseContentPage implements Log {
  private static EPPayDepositMove ep = EPPayDepositMove.getInstance();
  private static PayDepositMoveLogViewPage instance = null;

  public static PayDepositMoveLogViewPage getInstance() {
    if (instance == null)
      instance = new PayDepositMoveLogViewPage();
    return instance;
  }

  public PayDepositMoveLogViewPage() {
    super();
    drawToolbar();
    drawSelf();
  }
  ...
  private boolean checkIn() {
    if (ep.isProcessMode())
      return true;
    if (ep.isPermitted(PayDepositMovePermDef.READ) == false) {
  ...
  private void refreshTitle() {
    ...
    ep.getTitleBar().setTitleText(PayDepositMoveUrlParams.MODULE_CAPTION);
    if (entity != null) {
