 ...
 * 
 */
public class RecInvoiceRegLoader {

  private static EPRecInvoiceReg ep = EPRecInvoiceReg.getInstance();
  ...
  public static void decodeParams(JumpParameters params, final Command callback) {
    ...
    if (ep.isProcessMode()) {
      ...
      billUuid = params.getUrlRef().get(RecInvoiceRegUrlParams.Base.PN_UUID);
      billNumber = params.getUrlRef().get(RecInvoiceRegUrlParams.Base.PN_BILLNUMBER);
    }
    entity = null;
  ...
  private static void doLoad(String billUuid, final Command callback) {
    RLoadingDialog.show();
    RecInvoiceRegService.Locator.getService().load(billUuid, new RBAsyncCallback2<BInvoiceReg>() {
      @Override
      public void onException(Throwable caught) {
        RLoadingDialog.hide();
        String msg = InvoiceRegMessage.M.cannotFind(RecInvoiceRegUrlParams.MODULE_CAPTION);
  ...
  private static void doLoadByNumber(final String billNumber, final Command callback) {
    RLoadingDialog.show();
    RecInvoiceRegService.Locator.getService().loadByNumber(billNumber,
        new RBAsyncCallback2<BInvoiceReg>() {
          ...
          public void onSuccess(BInvoiceReg result) {
            ...
            if (result != null) {
              ...
              String msg = InvoiceRegMessage.M.cannotFind2(RecInvoiceRegUrlParams.MODULE_CAPTION,
                  billNumber);
              RMsgBox.showErrorAndBack(msg, null);
            }
